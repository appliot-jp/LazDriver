...
// The array that stores the round eteys.
...
static void KeyExpansion(void)
{
	...
	for(; (i < (Nb * (Nr + 1))); ++i)
	{
		...
		RoundKey[i * 4 + 0] = (uint8_t)(RoundKey[(i - Nk) * 4 + 0] ^ tempa[0]);
		RoundKey[i * 4 + 1] = (uint8_t)(RoundKey[(i - Nk) * 4 + 1] ^ tempa[1]);
		RoundKey[i * 4 + 2] = (uint8_t)(RoundKey[(i - Nk) * 4 + 2] ^ tempa[2]);
		RoundKey[i * 4 + 3] = (uint8_t)(RoundKey[(i - Nk) * 4 + 3] ^ tempa[3]);
	}
}
...
static void MixColumns(void)
{
	...
	{  
		t   = (*state)[i][0];
		Tmp = (uint8_t)((*state)[i][0] ^ (*state)[i][1] ^ (*state)[i][2] ^ (*state)[i][3]) ;
...
{
	uintptr_t i;
	uint8_t remainders = (uint8_t)(length % KEYLEN); /* Remaining bytes in the last non-full block */
	...
	for(i = 0; i < (uint8_t)length; i += KEYLEN)
	{
		XorWithIv(input);
		BlockCopy(output, input);
		state = (state_t*)output;
...
uint8_t AES128_CBC_encrypt(uint8_t* output, uint8_t* input, uint32_t length, uint8_t index)
{
	...
	for(i=0; i < 16; i++) {
		*(ivTable+i) = (uint8_t)(*(p_key+i) ^ index);
	}

	memcpy(workspace, input, (size_t)length);

	padLen = (uint8_t)((~(length % 16) + 1)&0x0f);
...
uint8_t AES128_CBC_decrypt(uint8_t* output, uint8_t* input, uint32_t length, uint8_t index)
{
	...
	memcpy(output, workspace, (size_t)(length-padLen));
