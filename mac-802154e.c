...
static int addr_comp(t_MAC_ADDR *pAddr)
{
	    ...
	    (pAddr->panid_enb == 0))
	{
		...
		{
		case 1:
			if((pAddr->addr.addr8 == macParam.myInfo.myAddr.addr8)||(pAddr->addr.addr8 == global_addr.addr8)) result = true;
			DEBUGONDISPLAY(MODE_MAC_DEBUG, printk("myAddr=%02x,rxAddr=%02x\n", pAddr->addr.addr8, macParam.myInfo.myAddr.addr8));
			break;
		case 2:
			if((pAddr->addr.addr16 == macParam.myInfo.myAddr.addr16)|| (pAddr->addr.addr16 == global_addr.addr16))result = true;
			DEBUGONDISPLAY(MODE_MAC_DEBUG, printk("rxAddr=%04x,myAddr=%04x\n", pAddr->addr.addr16, macParam.myInfo.myAddr.addr16));
			break;
		case 3:
			if((memcmp(pAddr->addr.addr64, macParam.myInfo.myAddr.addr64,8)==0)|| (memcmp(pAddr->addr.addr64, global_addr.addr64,8)==0)) result = true;
