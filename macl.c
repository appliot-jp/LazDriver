...
#ifndef LAZURITE_IDE
		ACCESS_POP();
		//printk(KERN_INFO"%s %d %d %d\n",__func__,__LINE__,macl.condition,macl.hopping_state);
#endif
	} else {
...
static void macl_ack_rxdone_handler(void) {
	...
	switch(rxtype) {
		case FIFO_CONT:
			break;
		case FIFO_DONE:
			if(((macl.phy->in.data[0]&0x07) == IEEE802154_FC_TYPE_ACK) && (macl.phy->in.data[2] == macl.phy->out.data[2])){
				phy_timer_stop();
				macl_rx_irq(&isAck);
...
int	macl_xmit_sync(BUFFER *buff) {
	...
	//printk(KERN_INFO"%s %d %d %d %d %d\n",__func__,__LINE__,time,macl.condition,macl.rxdone,macl.hoppingdone);
